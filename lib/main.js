...
var pageConstants = [
    ...
    ["furaffinity", new RegExp("^https?:\/\/(?:[^.]+\.)?furaffinity\.net\/view\/[0-9]+"), "*.furaffinity.net"],
    ["deviantart", new RegExp("^https?:\/\/(?:[^.]+\.)?deviantart\.com\/art\/.+"), "*.deviantart.com"],
    ["inkbunny", new RegExp("^https?:\/\/inkbunny\.net\/submissionview\.php\/.+"), "*.inkbunny.net"],
    ["sofurry", new RegExp("^https?:\/\/www\.sofurry\.com\/view\/.+"), "*.sofurry.com"]
];
...
// Page Mods
for each (var siteInfo in pageConstants) {
	console.log("Creating pageMod for site " + siteInfo[0]);
	pageMod.PageMod({
		include: siteInfo[2],
		contentScriptFile: [data.url('jquery-2.0.3.min.js'), data.url(siteInfo[0]+".js")],
